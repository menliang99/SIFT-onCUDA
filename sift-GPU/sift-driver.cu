...
#define RAW_PT(x)       reinterpret_cast<char*>(x)
  ...
  if( fp ) {
    ...
    for(int i = 0 ; i < 128 ; ++i)
      idescr_pt[i] = VL::uint8_t(VL::float32_t(512) * descr_pt[i]) ;
...
main(int argc, char** argv)
{
  ...
  while( argc > 0 ) {
	...
	throw VL::Exception("PGM read error: "+e.msg) ;
      ...
      if(O < 1) {
	O = std::max( (int)(std::floor(log2((double)std::min(buffer.width, buffer.height))) - omin -3), 1);
      }
    ...
    CUT_SAFE_CALL(cutCreateTimer(&timer));
      ...
      VL::Sift sift(buffer.data, buffer.width, buffer.height, 
		    ...
		    omin, -1, S+1) ;
      cutStopTimer(timer);
      Gau_time = cutGetTimerValue(timer);   
      verbose && cout 
        << "siftpp: Gaussian scale space completed."<<" Cost time is "<<Gau_time<<" ms."
        << endl ;
...
/////////////////////////////////////////GPU program below////////////////////////////////////////////////   
		...
		cutStartTimer(timer);     
            ...
            int totalK = sift.keypointsEnd() - sift.keypointsBegin(); 

	    VL::Sift::Keypoint  Hkvalue[totalK];
	    int i = 0;
	    for( VL::Sift::KeypointsConstIter iter = sift.keypointsBegin() ; iter != sift.keypointsEnd() ; ++iter ) {
		
		Hkvalue[i].ix = iter->ix;
		Hkvalue[i].iy = iter->iy;
		Hkvalue[i].is = iter->is;
		...
		Hkvalue[i].o = iter->o;
		Hkvalue[i].sigma = iter->sigma;
		i++;

//std::cout<<"Save Kvalue to HalfKvalue  "<<iter->x<<std::endl;
	    }

	      std::cout<<"Save Kvalue to HalfKvalue"<<"   total key number is "<<i<<endl;
     	      int addr_mark = 0;

	   for(int o = omin; o < omin + O; ++o) 
		{

	   	VL::Sift::Keypoint* K_addr = &Hkvalue[addr_mark];
	     	//int layers = smax - smin - 2;    // layers = S+1-(-1)-2 = S
		int width = sift.getOctaveWidth(o);
		int height = sift.getOctaveHeight(o);
		VL::float_t xperiod = sift.getOctaveSamplingPeriod(o) ;
    		//int square = 2 * width * height;
           	unsigned int k_size = sizeof(VL::Sift::Keypoint) * (sift.KeyNum[o-omin]);
           	unsigned int Ok_size = sizeof(VL::OKvalue) * (sift.KeyNum[o-omin]);
		unsigned int grad_size = sizeof(pixel_t) * (2*S*width*height);

        	pixel_t* Grad = sift.temp;
		//Grad = new pixel_t [2*S*width*height];
		OKvalue*  Kdst;
    		Kdst = new OKvalue [sift.KeyNum[o-omin]];	
		sift.GradinGpu(Grad, o, width, height);

		pixel_t* Grad_d = NULL;
		VL::Sift::Keypoint* Kin_d = NULL;
		VL::OKvalue* Kout_d = NULL;

    		CUDA_SAFE_CALL( cudaMalloc( (void**) &Kout_d, Ok_size));  
    		CUDA_SAFE_CALL( cudaMalloc( (void**) &Kin_d, k_size));
    		CUDA_SAFE_CALL( cudaMalloc( (void**) &Grad_d, grad_size));


    		dim3 dimBlock, dimGrid;
    		dimBlock.x = K_BLOCK_SIZE;
 
    		dimGrid.x = ( sift.KeyNum[o-omin] / K_BLOCK_SIZE) + ((sift.KeyNum[o-omin] % K_BLOCK_SIZE) ? 1:0 );


		CUDA_SAFE_CALL( cudaMemcpy(Grad_d, Grad, grad_size, cudaMemcpyHostToDevice) );
		CUDA_SAFE_CALL( cudaMemcpy(Kin_d, K_addr, k_size, cudaMemcpyHostToDevice) );


      		GetkKernel<<<dimGrid, dimBlock>>>(Kin_d, Grad_d, Kout_d, sift.KeyNum[o-omin], width, height, -1, xperiod, magnif);

      		CUDA_SAFE_CALL(cudaMemcpy(Kdst, Kout_d, Ok_size, cudaMemcpyDeviceToHost));


		for(int i = 0; i < sift.KeyNum[o-omin]; i ++)
		    for(int a = 0 ; a < Kdst[i].nangles ; ++a)
		      {

		  	    out << setprecision(2) << (K_addr+i)->x << ' '
		  		<< setprecision(2) << (K_addr+i)->y << ' '
		  		<< setprecision(2) << (K_addr+i)->sigma << ' '
		  		//<< setprecision(2) << (K_addr+i)->o << ' ';
		  		<< setprecision(3) << Kdst[i].th[a] ;

				float*  descr_pt = &(Kdst[i].descr_pt[0]) + 128*a;
			...
			//delete[] Grad;
			delete[] Kdst;
			addr_mark += sift.KeyNum[o-omin];	

		}

      		cutStopTimer(timer);
	        com_time = cutGetTimerValue(timer)/1000;
    		CUT_SAFE_CALL(cutDeleteTimer(timer));

	verbose && cout 
          << "siftpp:   finish description of all key points. Cost time is "<<com_time<<" s."<<endl ;

	}

    
//////////////////////////////////change CPU and GPU here//////////////////////////////////////////
/*
	  // -------------------------------------------------------------
